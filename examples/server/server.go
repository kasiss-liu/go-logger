...
import (
	...
	"github.com/kasiss-liu/go-configurator"
	"github.com/kasiss-liu/go-logger"
)

func main() {
	//读取配置 获取到writers
	config := configurator.New("loggers", "./config/server.json")
	...
	for _, m := range writers {
		if writer, ok := m.(map[string]interface{}); ok {
			...
			switch wType {
			case "file":
				res := logger.RegisterFileLogger(mark, dir, prefix, int(mode))
				if !res {
...
func setDataToChan(conn net.Conn) (err error) {
	...
	for {
		...
		case "LOG":
			err = logger.Log(mark, content, timeNow)
		case "NOTICE":
			err = logger.Notice(mark, content, timeNow)
		case "WARNING":
			err = logger.Warning(mark, content, timeNow)
		case "ERROR":
			err = logger.Error(mark, content, timeNow)
		default:
			err = logger.Write(mark, stype, content, timeNow)
		}
